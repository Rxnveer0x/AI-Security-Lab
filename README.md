...
AI Security Lab started as a simple local AI chatbot and was later expanded with different security controls.

The main idea is simple: user requests are checked before they reach the AI model, and the AI response is checked before it is sent back to the user.
...
The AI model runs locally through **Ollama**, so the application does not need a cloud AI API for its basic operation.
...
The project also has a basic RAG security layer for working with documents.
...
Retrieved content is checked before it is passed further into the AI workflow.

### Tool Security

A calculator tool is included to show how AI tool requests can be controlled.
...
The calculator only accepts expressions that match the allowed format instead of allowing arbitrary input to be executed.

### Security Monitoring

Security events are recorded so they can be checked later through the dashboard.
...
The project includes a web-based security dashboard for monitoring the application.

Instead of checking the security log manually, the dashboard provides the main security information in one place.
...
The following examples show how some of the security controls work in the application.
...
If a jailbreak attempt is detected, the request is blocked and recorded as a security event.
...
If sensitive information is detected, the request can be blocked before reaching the AI model.
...
Retrieved documents are also checked for suspicious instructions before being used by the application.
...
Tool requests are sent through the tool router and validation layer before they are executed.
...
The response generated by the AI model is checked before being returned to the user.
...
Important security events are written to the security log and then shown through the dashboard.
...
The normal pytest suite currently contains **44 tests**, and all of them pass.
...
There are also separate attack-testing scripts for checking the security controls against different malicious or suspicious inputs.
...
The project includes a GitHub Actions workflow that runs the security tests automatically.
...
The workflow installs the project dependencies and runs the pytest test suite.
...
I didn't want to stop at making a basic chatbot. I wanted to understand what happens when security controls are added around an AI application and how different types of AI-related threats can be detected and handled.
